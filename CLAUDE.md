...
Resilience: **soft fallback** — when no chunks clear `COSINE_THRESHOLD` (0.15), `rerank_node` sets `fallback_chunks` instead of returning nothing; `generate_node` uses these with a ⚠️ disclaimer rather than hard-declining. **Intent-aware output validation** — `validation_node` runs a citation existence check (no LLM) then an LLM grounding check against per-intent quality rubrics (definition / how-to / advisory / comparison), returning `verdict: pass | warn | fail` with `reason` and `suggestion`; in the Streamlit streaming path `validate_answer()` is called after streaming completes and surfaces `st.warning()` or `st.error()` as appropriate. **Feedback loop** — thumbs up/down in Streamlit UI stored via `FeedbackStore`; repeated negatives on the same query invalidate the Q&A cache.

Supporting modules under `ciathena_kb/`:

- **`loader.py`** — parses each artifact YAML into an `Artifact(envelope, body)`. Files may use `---` separators; all YAML docs in a file are *merged* into one mapping, then split into envelope vs body by the known `ENVELOPE_KEYS` set. **Relaxed validation**: only `artifact_id` + `component_type` are hard-required; other envelope keys (`title`, `usecase`, `layer`, etc.) are recommended with soft warnings. Controlled vocabularies (layers, component_types, review_status) are advisory — unknown values are accepted with a warning so new artifact generations don't block ingestion. Includes **YAML repair** for common hand-authoring defects (missing colon-space after mapping keys). Also provides `load_artifact_from_bytes()` for parsing from blob downloads.
- **`chunker.py`** — the **universal chunking rule**: one item under an artifact's body list = exactly one `Chunk`. Whole-doc types are **not** chunked. Embedded text = human-readable fields **plus recall boosters** (`synonyms`, `disambiguation_triggers`, `trigger_patterns`). Envelope fields become filterable metadata. **Auto-detect for unknown component_types**: `_detect_list_key()` finds the body list by convention or largest-list heuristic; `_detect_id_field()` picks the first `*_id` field from items. Known types still use curated field mappings.
...
- **`query_expander_node.py`** — LLM generates 3 semantically diverse query variations from the rewritten query. Sets `state["expanded_queries"]` for multi-query retrieval. Skips when `FakeChatLLM` or out-of-domain. Accepts optional `system_prompt` override (editable via Streamlit UI prompt manager).
- **`validation_node.py`** — post-generation quality check. (1) Citation existence check (no LLM): flags `[artifact::chunk]` refs not in the effective context. (2) **Intent-aware LLM grounding check**: evaluates the answer against `INTENT_RUBRICS` (definition / how-to / advisory / comparison) and returns a 3-tier `verdict: "pass" | "warn" | "fail"` with `reason` and `suggestion`. State key: `state["validation_result"] = {"verdict", "passed", "issues", "reason", "suggestion"}`. Skipped for fallback answers (already disclaimed). Also exposes `validate_answer()` as a standalone callable (used by Streamlit streaming path). Accepts optional `system_prompt` override.
- **`agent_graph.py`** — assembles the full LangGraph with conditional edges: in_domain check (skip generation if out-of-domain) and rerank bypass (skip LLM grading when all top-k chunks are high-confidence). Accepts optional `prompts` dict to pass prompt overrides to all nodes (now including `query_expander` and `validation_grounding` keys). Also provides `build_pre_generate_graph()` that stops after rerank (used with streaming). Streamlit UI uses `graph.stream()` for progressive status updates.
...
- **`prompt_manager.py`** — manages all 5 pipeline prompt templates: `router_system`, `rerank_grading`, `generate_system`, `query_expander`, `validation_grounding`. Loads from blob when available, falls back to built-in defaults. All 5 prompts are editable via Streamlit UI sidebar without redeploying. Router prompt includes `chroma_filter` field in JSON schema (instruction 7). Validation prompt uses `{intent}`, `{rubric}`, `{question}`, `{chunks}` placeholders.
